...
var Ask = {
  ...
  init: function() {
    var ping_url_el = Ask.$('ask-ping-url');

    if ( ! ping_url_el || ! Ask.$('ask-chat') ) {
      return;
    }

    Ask.ping_url = ping_url_el.title;

    Ask.status_update();

    Ask.status_update_interval = setInterval( Ask.status_update, 60000 );

    Ask.$('ask-email-link').onclick = function() {
      clearInterval( Ask.status_update_interval );
      Ask.init_email_link();
      return false;
    };

    Ask.$('ask-email-form').onsubmit = function() {
      if ( ! Ask.validate_email( Ask.$('ask-email-form-email').value ) ) {
        $('#ask-email-error').text( 'Please enter a valid email address.' );
        $('#ask-email-error').show();
        return false;
      }

      if ( ! Ask.$('ask-email-form-message').value ) {
        $('#ask-email-error').text( 'Please enter a message.' );
        $('#ask-email-error').show();
        return false;
      }
    ...
    Ask.$('ask-chat-form').onsubmit = function() {
      if ( ! $('#ask-chat-form-message').val() ) {
        $('#ask-chat-error').text( 'Please enter a message.' );
        $('#ask-chat-error').show();
        return false;
      }

      var email_address = Ask.$('ask-chat-form-email').value;

      if ( email_address && ! Ask.validate_email( email_address ) ) {
        $('#ask-chat-error').text( 'Please enter a valid email address.' );
        $('#ask-chat-error').show();
        return false;
      }
    ...
    var config = {
      sensitivity: 3,
      interval: 200,
      over: Ask.overlay_show,
      timeout: 1200,
      out: Ask.overlay_hide
    };

    Ask.$('ask-chat-unavailable').title = '';
    $('#ask-chat-unavailable').hoverIntent( config );
  },

  chat_available: function() {
    Ask.$('ask-chat-link').style.display = 'inline';
    Ask.$('ask-chat-unavailable').style.display = 'none';

    Ask.$('ask-chat-link').onclick = function() {
      clearInterval( Ask.status_update_interval );
      Ask.init_chat_link();
      return false;
    };
  ...
  chat_unavailable: function() {
    Ask.$('ask-chat-link').style.display = 'none';
    Ask.$('ask-chat-unavailable').style.display = 'inline';
    Ask.$('ask-chat-form').onsubmit = null;
    Ask.$('ask-chat-link').onclick = null;
  },

  init_email_link: function () {
    $('#ask-chat').slideUp( 'fast' );
    $('#ask-email').slideToggle( 'fast' );
  },

  init_chat_link: function () {
    $('#ask-email').slideUp( 'fast' );
    $('#ask-chat').slideToggle( 'fast' );
  },

  overlay_hide: function() {
    $('#ask-chat-hours-overlay').fadeOut();
  },

  overlay_show: function() {
    $('#ask-chat-hours-overlay').fadeIn();
  },
    ...
    var xhr = $.ajax({
      ...
      success: function( the_data ) {
        if ( the_data === 'true' ) {
          ...
          Ask.chat_unavailable();
        }
      },
      error: Ask.chat_unavailable
    });
  },
...
;
function tc1_sersol_code( library_code )
{
  switch ( library_code )
  {
    case 'TCNY':
...
function doFaqToc()
{
  ...
  h2s.each( function() {
    var text = $(this).text();
    var snippeted = snippet( text );
    this.id = 'faq-' + snippeted;
    toc.append( '<li><a href="#faq-' + snippeted + '">' + text + '</a></li>' );
  });
}

function archiveSeriesList()
{
  branch_items = $('#node-156 > div > ul > li');
  child_lists  = $('#node-156 > div > ul > li > ul');

  child_lists.hide().addClass('javascripted-child-list');
  $('#node-156 > div > ul > li > ul > li').addClass('javascripted-leaf');

  branch_items.addClass('unexpanded-branch');

  branch_items.toggle( function() {
    $(this).removeClass('unexpanded-branch');
    $(this).addClass('expanded-branch');
    $(this).children('ul').slideDown();
  }, function() {
    $(this).removeClass('expanded-branch');
    $(this).addClass('unexpanded-branch');
    $(this).children('ul').slideUp();
  });

  branch_items.children('ul').click( function() {return false;} );
}

function newFrontLinksAdjust()
{
  if ( ! $('.newhome').length ) {
    return;
  }

  b1 = $('#homepage #block-menu_block-1');
  b2 = $('#homepage #block-menu_block-2');
  b3 = $('#homepage #block-menu_block-3');

  w_available = $('#homepage #homepage-column-left').width();
  w_total     = b1.outerWidth() + b2.outerWidth() + b3.outerWidth();
  w_each_max  = Math.floor( w_available / 3 );

  if ( w_total == w_available ) {
    return;
  }
  else if ( w_total < w_available ) {
    w_each_pad = Math.floor( (w_available - w_total) / 3 );

    b1.width( b1.width() + w_each_pad );
    b2.width( b2.width() + w_each_pad );
    b3.width( b3.width() + w_each_pad );
  }
  else {
    to_change = [];

    if ( b1.outerWidth() > w_each_max ) {
      to_change.push( b1 );
    }
    if ( b2.outerWidth() > w_each_max ) {
      to_change.push( b2 );
    }
    if ( b3.outerWidth() > w_each_max ) {
      to_change.push( b3 );
    }

    w_each_reduce = Math.ceil( (w_total - w_available) / to_change.length );

    for ( key in to_change ) {
      el = to_change[ key ];
      el.width( el.width() - w_each_reduce );
    }
  }
...
$(document).ready( function() {
  var tab_from_cookie = $.cookie('ui-tabs-1');
  var tab_selected;

  if ( tab_from_cookie !== null ) {
    tab_selected = parseInt( tab_from_cookie );
  }
  else {
    tab_selected = 1;
  }
  ...
  if ( library_code_url_el ) {
    library_code_url = library_code_url_el.title;

    $.ajax({
      url: library_code_url_el.title,
      success: function( the_data ) {
        var library_code = the_data;

        if (
          library_code.match(/^[A-Z0-9-]+$/)
          && library_code != 'TCNY'
          && library_code != 'TUI'
        ) {
          document.getElementById('homepage-find-quick-yes').style.display = 'none';
          document.getElementById('homepage-find-quick-no').style.display = 'block';
        }

        document.getElementById('homepage-find-form-ejournals')
          .action = 'http://' + tc1_sersol_code( library_code ) + '.search.serialssolutions.com';

        if ( ! library_code ) {
          document.getElementById('homepage-find-ejournals-options-outside')
  ...
  var ejournals_options = false;

  $('#homepage-find-ejournals-options-link').click( function() {
    if ( ejournals_options ) {
      $('#homepage-find-ejournals-options').slideUp();
      $('#homepage-find-ejournals-options-link').text('More options');
      ejournals_options = false;
    }
    else {
      $('#homepage-find-ejournals-options').slideDown();
      $('#homepage-find-ejournals-options-link').text('Hide options');
      ejournals_options = true;
    }
  });
  ...
  $('.homepage-find-text-primary').change( function() {
    $('.homepage-find-text-primary').val( $(this).val() );
  });

  $('#homepage-databases-form-subject').change( function() {
    $(this).parent().submit();
  });

  if ( $.cookie('force_classic_theme') ) {
    var footer_links = document.getElementById( 'footer-links' );

    if ( footer_links != null ) {
      var domain_matches = window.location.host.match(/\..+$/);

      var href = window.location.protocol + '//m' + domain_matches + window.location.pathname;

      var mobile_link = $('<div><a href="' + href + '">Switch to Mobile View</a></div>');

      mobile_link.click( function() {
        document.cookie = 'force_classic_theme=false; expires=Fri, 13 May 2011 14:40:26 GMT; domain=' + domain_matches[0];
      })

      $(footer_links).after( mobile_link );
    }
  }
});
;
...
BrowserDetect.init();;
var touro_menubar_item;
var touro_menubar_header;

function touro_menubar_open()
{
  touro_menubar_close();
  touro_menubar_item = $(this).find('ul').show();
  touro_menubar_header = $(this).find('h3').addClass( 'hover' );
}

function touro_menubar_close()
{
  if (touro_menubar_item) {
    touro_menubar_item.hide();
    touro_menubar_header.removeClass( 'hover' );
  }
}

$(window).load( function() {
  container_width = document.getElementById('touro-menubar-container')
                    .clientWidth;

  menubar_width = 0;
  $('#touro-menubar > li').each( function() {
    menubar_width += $(this).width();
  });

  switch (BrowserDetect.browser) {
    case 'Explorer':
      menubar_left = (container_width - menubar_width) / 2 - 1;
      $('#touro-menubar li h3').css('padding-right', '16px');
      $('#touro-menubar li h3').css('padding-left', '17px');
      break;

    case 'Safari':
      menubar_left = (container_width - menubar_width) / 2 + 6;
      break;

    case 'Chrome':
      menubar_left = (container_width - menubar_width) / 2 + 5;
      break;

    case 'Firefox':
      menubar_left = (container_width - menubar_width) / 2 + 13;
      $('#touro-menubar li h3').css('padding', '5px 15px 4px 18px');
      break;

    case 'Konqueror':
      menubar_left = (container_width - menubar_width) / 2 + 14;
      $('#touro-menubar li h3').css('padding', '5px 15px 4px 18px');
      break;

    case 'Opera':
    default:
      menubar_left = (container_width - menubar_width) / 2;
  }

  $('#touro-menubar').css('margin-left', menubar_left);
});

$(document).ready( function() {
  $('#touro-menubar-text-link').hide();

  $('#touro-menubar > li').bind('mouseover', touro_menubar_open)
  $('#touro-menubar > li').bind('mouseout',  touro_menubar_close)
  $('#touro-menubar').show();
});

document.onclick = touro_menubar_close;
